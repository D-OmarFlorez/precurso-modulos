...
function numberOfCharacters(string) {
   ...
   for (let i = 0; i < string.length; i++) {
     ...
     conteo[letras] = (conteo[letras] || 0) + 1;
   }
   const conteoDeLetras = {};
   Object.keys(conteo).sort().forEach(key => {
     conteoDeLetras[key] = conteo[key];
   });
   return conteoDeLetras;
 }    
...
function asAmirror(frase) {
   ...
   const palabrasInvertidas = palabras.map(palabra => invertirPalabra(palabra));
